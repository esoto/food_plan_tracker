...
RSpec.describe ReminderPreference, type: :model do
  ...
  describe ".enabled?" do
    ...
    it "is false when a row says so" do
      described_class.create!(reminder_type: "meal", key: "Breakfast", enabled: false)
    ...
    it "is true when a row explicitly enables it" do
      described_class.create!(reminder_type: "meal", key: "Lunch", enabled: true)
  ...
  describe "validations" do
    it "rejects an unknown reminder_type" do
      pref = described_class.new(reminder_type: "made_up", key: "x", enabled: true)
      expect(pref).not_to be_valid
      expect(pref.errors[:reminder_type]).to be_present
    end

    it "rejects duplicate (reminder_type, key)" do
      described_class.create!(reminder_type: "meal", key: "Breakfast", enabled: true)
      dupe = described_class.new(reminder_type: "meal", key: "Breakfast", enabled: false)
