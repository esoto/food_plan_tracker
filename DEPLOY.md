...
4. Runs `bin/docker-entrypoint`, which executes `db:prepare` against all four
   production databases. The schema files
   [`db/queue_schema.rb`](db/queue_schema.rb),
   [`db/cache_schema.rb`](db/cache_schema.rb),
   [`db/cable_schema.rb`](db/cable_schema.rb) — generated by
   `bin/rails solid_{queue,cache,cable}:install` and committed to the repo —
   load on first run via the `schema_dump:` directives in `config/database.yml`.
   On the very first deploy (when DBs don't exist) `db:prepare` also runs
   `db:seed`, creating the admin user from `ADMIN_EMAIL`/`ADMIN_PASSWORD`.
   Re-seed manually any time with `kamal seed` (alias for `bin/rails db:seed`,
   which is idempotent).
5. Registers `food.estebansoto.dev` with the shared `kamal-proxy`; Let's
   Encrypt issues an SSL cert.
6. Starts the container; Solid Queue boots inside Puma.
...
Manual snapshot via SSH (look up the active container name first via
`docker ps`; Kamal names it `<service>-web` plus a generation suffix):

```bash
CONTAINER=$(ssh deploy@178.104.88.183 "docker ps --filter name=food_plan_tracker-web --format '{{.Names}}' | head -1")
ssh deploy@178.104.88.183 "docker exec ${CONTAINER} sqlite3 /rails/storage/production.sqlite3 .dump" \
  > backup-$(date +%Y%m%d).sql
[ -s backup-*.sql ] || echo "WARNING: backup is empty — wrong container name?"
```
