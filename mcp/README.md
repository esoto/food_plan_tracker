...
| `list_meal_items` / `add_meal_item` / `update_meal_item` / `remove_meal_item` | Edit the food items inside a meal — change a portion, swap a stacked fat source, etc. Lookup by `meal_name` + `food_name`. |
